                    ...
                    <div className="flex flex-wrap gap-1.5 min-h-[40px] p-2 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-800">
                      {tempSubjects.length === 0 ? (
                        ...
                        tempSubjects.map((sub, idx) => (
                          <span key={`${sub}_${idx}`} className="inline-flex items-center gap-1.5 bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 px-2 py-1 rounded-lg text-xs font-bold border border-blue-100/30">
                            <span>{sub}</span>
                            <button
                              type="button"
                              onClick={() => setTempSubjects(tempSubjects.filter((s, i) => i !== idx))}
          ...
          <div className="flex flex-col gap-4 animate-fadeIn">
            ...
            <div className="flex flex-col gap-1.5 mt-1">
              ...
              <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-none">
                {sortedEnrolledSubjects.map((subject, idx) => {
                  const percent = collectiveProgressStats.breakdown[subject]?.percent || 0;
