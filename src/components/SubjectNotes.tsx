...
import { groupUPSCNotesHierarchy } from "../utils/upscHierarchyHelper";
...
export default function SubjectNotes({
  ...
  // Collapsible module & chapter state
  const [expandedModules, setExpandedModules] = useState<Record<string | number, boolean>>({});
  const [expandedChapters, setExpandedChapters] = useState<Record<string | number, boolean>>({});

  const toggleModuleExpand = (moduleKey: string | number) => {
  ...
  const toggleChapterExpand = (chapterKey: string | number) => {
  ...
  // Determine if this is UPSC class
  const isUPSC = useMemo(() => {
    if (normalizeClassGrade(classGrade) === "UPSC") return true;
    const target = students?.find(
      (s) => s.id === studentId || (s.name && studentName && s.name.toLowerCase() === studentName.toLowerCase())
    );
    return normalizeClassGrade(target?.classGrade || "") === "UPSC";
  }, [classGrade, students, studentId, studentName]);
  ...
  // 2. Filter notes by Subject, GS Paper, Module, Chapter Number/Name, Topic, or PDF file name
  const filteredNotes = useMemo(() => {
    ...
    return accessibleNotes.filter((note) => {
      ...
      const matchGS = gs.includes(query);
      ...
      const matchMod = modNo.includes(query) || modName.includes(query);
      ...
      const matchTopic = topNo.includes(query) || topName.includes(query);
      const matchFileName = (note.pdfFileName || "").toLowerCase().includes(query);
      return matchSubject || matchGS || matchMod || matchChNo || matchChName || matchTopic || matchFileName;
    });
  }, [accessibleNotes, searchQuery, subject]);

  // Grouping for Non-UPSC classes: Chapter -> Topics
  const groupedChapterNotes = useMemo(() => {
    return groupAndSortChapterNotes(filteredNotes);
  }, [filteredNotes]);

  // Grouping for UPSC class: GS Paper -> Subject -> Module -> Chapter -> Topics
  const upscHierarchy = useMemo(() => {
    if (!isUPSC) return [];
    return groupUPSCNotesHierarchy<ChapterNote>(filteredNotes);
  }, [filteredNotes, isUPSC]);

  // Flattened UPSC Modules for current subject view
  const upscCurrentSubjectModules = useMemo(() => {
    if (!isUPSC || upscHierarchy.length === 0) return [];
    const normSubj = subject.toLowerCase().trim();
    for (const gs of upscHierarchy) {
      const matchSub = gs.subjects.find((s) => s.subject.toLowerCase().trim() === normSubj);
      if (matchSub) {
        return matchSub.modules;
      }
    }
    // If not matching exact subject, collect all modules from all subjects in upscHierarchy
    return upscHierarchy.flatMap((gs) => gs.subjects.flatMap((s) => s.modules));
  }, [upscHierarchy, isUPSC, subject]);

  // Inferred or detected GS Paper name for breadcrumb/header
  const currentGSPaper = useMemo(() => {
    if (!isUPSC) return "";
    const firstNoteWithGS = filteredNotes.find((n) => n.generalStudiesPaper || (n as any).gs_paper);
    if (firstNoteWithGS) {
      return firstNoteWithGS.generalStudiesPaper || (firstNoteWithGS as any).gs_paper;
    }
    return inferGSPaperFromSubject(subject);
  }, [isUPSC, filteredNotes, subject]);
      ...
      <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-4" id="notes-header">
        ...
        <div className="flex flex-col">
          <span className="text-[10px] font-bold uppercase text-slate-400 dark:text-slate-500 tracking-wider">
            {isUPSC ? (
              <span>UPSC {currentGSPaper ? `• ${currentGSPaper}` : ""} • Subject Notes</span>
            ) : (
              <span>{studentName ? `${studentName} • ` : ""}{classGrade ? `${classGrade} • ` : ""}Subject Notes</span>
            )}
          </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-[200px_1fr] gap-6 items-start" id="notes-two-panel-container">
        ...
        <div className="flex flex-col gap-4 min-w-0" id="notes-right-content-panel">
          ...
          <div className="flex flex-col gap-3.5" id="notes-list-container">
            {isUPSC && upscCurrentSubjectModules.length > 0 ? (
              upscCurrentSubjectModules.map((modGroup) => {
                const modKey = `mod_${modGroup.moduleNo}`;
                const isModExpanded = expandedModules[modKey] !== false;

                return (
                  <div
                    key={modKey}
                    className="bg-white dark:bg-slate-900 rounded-2xl border border-amber-200/80 dark:border-amber-900/60 shadow-xs flex flex-col overflow-hidden transition-all"
                  >
                    {/* Module Header */}
                    <div
                      className="p-4 bg-amber-50/50 dark:bg-amber-950/20 flex items-center justify-between cursor-pointer hover:bg-amber-100/60 dark:hover:bg-amber-950/40 transition-colors group/mod"
                      onClick={() => toggleModuleExpand(modKey)}
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="p-1.5 bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 rounded-lg group-hover/mod:bg-amber-200 transition-colors shrink-0">
                          {isModExpanded ? (
                        ...
                        <h2 className="font-extrabold text-slate-850 dark:text-slate-100 text-sm break-words">
                          {modGroup.moduleTitle}
                        </h2>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-amber-700 dark:text-amber-300 bg-amber-100 dark:bg-amber-900/50 px-2.5 py-1 rounded-lg">
                          {modGroup.chapters.length} {modGroup.chapters.length === 1 ? "Chapter" : "Chapters"}
                        </span>
                      </div>
                    </div>
                    ...
                    {isModExpanded && (
                      <div className="p-3.5 space-y-3 bg-slate-50/40 dark:bg-slate-950/30 border-t border-amber-100 dark:border-amber-900/40">
                        {modGroup.chapters.map((chGroup) => {
                          const chKey = `mod_${modGroup.moduleNo}_ch_${chGroup.chapterNo}`;
                          const isChExpanded = expandedChapters[chKey] !== false;

                          return (
                            <div
                              key={chKey}
                              className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-2xs overflow-hidden"
                            >
                              {/* Chapter Header */}
                              <div
                                className="p-3.5 flex items-center justify-between cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group/ch"
                                onClick={() => toggleChapterExpand(chKey)}
                              >
                                <div className="flex items-center gap-2.5 min-w-0">
                                  <div className="p-1 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-lg group-hover/ch:bg-blue-50 group-hover/ch:text-blue-600 transition-colors shrink-0">
                                    {isChExpanded ? (
                                  ...
                                  <div className="p-1.5 bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400 rounded-lg shrink-0">
                                    <BookOpen className="w-3.5 h-3.5" />
                                  </div>
                                  <h3 className="font-bold text-slate-800 dark:text-slate-100 text-xs sm:text-sm break-words">
                                    {chGroup.chapterTitle}
                                  </h3>
                                </div>
                              ...
                              {isChExpanded && (
                                <div className="p-3 pt-0 flex flex-col gap-2 border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/30 dark:bg-slate-950/20">
                                  {chGroup.topics.map((tItem) => {
                                    const note = tItem.note;
                                    return (
                                      <div
                                        key={note.id}
                                        ...
                                        <div className="flex items-center gap-3 min-w-0">
                                          ...
                                          <div className="flex flex-col min-w-0">
                                            ...
                                            <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                                              {!isFileNameRedundant(tItem.topicLabel, note.pdfFileName) && note.pdfFileName ? (
                                              ...
                                              <div className="flex items-center gap-1.5 shrink-0">
                                                ...
                                                {isAttempted && stats && (
                                                  <button
                                                    ...
                                                    className="p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 rounded-xl transition-all border border-slate-200 dark:border-slate-700 cursor-pointer min-h-[40px] flex items-center justify-center"
                                                    title="View Detailed Test Analytics & History"
                                                  >
